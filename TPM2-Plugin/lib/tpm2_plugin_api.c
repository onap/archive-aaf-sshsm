...
static bool sign_and_save(tpm_sign_ctx *ctx,  unsigned char *sig, int *sig_len) {
    ...
    printf("Compute message hash digest size : %ld \n", sizeof(digest));
    ...
    signature_len = signature.signature.rsassa.sig.t.size;
    printf("Signature length from signature structure size:  %d \n", signature_len);


    signature_len = sizeof(signature.signature.rsassa.sig.t.buffer);
    printf("Signature length from signature structure buffer:  %d \n", signature_len);
    
    sig_len = malloc(sizeof(int));
    if (!sig_len) {
        printf("Memory allocation for signature size failed \n");
        return -1;
    }
    memcpy(sig_len, &signature_len, sizeof(int));

    sig = malloc(signature_len);
    if (!sig) {
        printf("Memory allocation for signature plain text failed \n");
        return -1;
    }
    memcpy(sig, signature.signature.rsassa.sig.t.buffer, signature_len);
...
out:
    printf("signature length before sapi teardown:  %d \n", *sig_len);
    sapi_teardown_full(sapi_context);
    printf("signature length after sapi teardown:  %d \n", *sig_len);
