...
passphrase="$(cat ${ABRMD_DATA}/host_${TPM_NODE_NAME}/passphrase)"
echo "${passphrase}" | gpg --batch --yes --passphrase-fd 0 password.gpg
password="$(cat ${ABRMD_DATA}/host_${TPM_NODE_NAME}/password)"

# 2. Create the tpm_status.yaml file to report Success/Failure Flag along with error
touch tpm_status.yaml
chmod 755 tpm_status.yaml
...
error=$(echo "TPM Startup failed"); flag="0"; echo "flag:${flag}" >> tpm_status.yaml;
echo "error:${error}" >> tpm_status.yaml; exit 1; fi
echo ""
...
echo "flag:${flag}" >> tpm_status.yaml; echo "error:${error}" >> tpm_status.yaml;
...
echo "$error"; flag="0";  echo "flag:${flag}" >> tpm_status.yaml;
echo "error:${error}" >> tpm_status.yaml; exit 1; fi
echo ""
...
echo "$error"; flag="0";  echo "flag:${flag}" >> tpm_status.yaml;
echo "errror:${error}" >> tpm_status.yaml; exit 1;  fi
echo ""
rm -f PrimaryKeyBlob
...
echo "flag:${flag}" >> tpm_status.yaml
echo "error:${error}" >> tpm_status.yaml
...
cp tpm_status.yaml ${ABRMD_DATA}/host_${TPM_NODE_NAME}
