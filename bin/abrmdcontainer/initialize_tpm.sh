...
# This is demarcate the generated files which are specific to this host's TPM
  mkdir -p ${ABRMD_DATA}/${TPM_NODE_NAME}
  touch ${ABRMD_DATA}/${TPM_NODE_NAME}/srkhandle.txt
  echo "0x81000011" >> ${ABRMD_DATA}/${TPM_NODE_NAME}/srkhandle.txt
  touch ${ABRMD_DATA}/${TPM_NODE_NAME}/password.txt
  echo "new" >> ${ABRMD_DATA}/${TPM_NODE_NAME}/password.txt
  srkhandle="$(cat ${ABRMD_DATA}/${TPM_NODE_NAME}/srkhandle.txt)"
  password="$(cat ${ABRMD_DATA}/${TPM_NODE_NAME}/password.txt)"
...
if [ $? -ne 0 ]; then echo; echo -e "${RED}Error, Exit."; error=$(echo" Error:Reading Public part of Primary Key failed"); echo "$error"; flag="0"; echo "flag:${flag}" >> flag.yaml; echo "error:${error}" >> flag.yaml; exit 1; fi
echo ""
...
# Use environment variable tpm_host_name is available else use onap as the sample host name
  cp out_parent_public ${ABRMD_DATA}/${TPM_NODE_NAME}
  cp flag.yaml ${ABRMD_DATA}/${TPM_NODE_NAME}
