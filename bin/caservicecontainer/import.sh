...
if [ ${sharedvolume}/out_parent_public ]; then

    mkdir -p /var/run/dbus
    # 2.a Copy the required input files for the Import tool
    cp ${sharedvolume}/dup* /sshsm/tpm-util/import/

    # 2.b Run the Import Utility
    cd /sshsm/tpm-util/import
    ./ossl_tpm_import -H $tpm_handle -dupPub dupPub -dupPriv dupPriv -dupSymSeed dupSymseed -dupEncKey dupEncKey -pub outPub -priv outPriv
